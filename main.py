...
class BMSApp:
    def __init__(self, root):
        ...
        self.root.state('zoomed')  # This makes the window open maximized
    ...
    def _create_main_layout(self):
        ...
        # Set all frames to expand and fill space
        ...
        # --- Min/Max Voltage Labels ---
        self.min_voltage_label = ttk.Label(controls_frame, text="Min Voltage: -- V", font=self.label_font)
        self.min_voltage_label.grid(row=3, column=0, padx=5, pady=5)
        self.avg_voltage_label = ttk.Label(controls_frame, text="Average Voltage: -- V", font=self.label_font)
        self.avg_voltage_label.grid(row=3, column=2, padx=5, pady=5)

        self.max_voltage_label = ttk.Label(controls_frame, text="Max Voltage: -- V", font=self.label_font)
        self.max_voltage_label.grid(row=3, column=1, padx=5, pady=5)
        ...
        # Configure grid for tab2 to expand with resizing
    ...
    def _update_gui(self):
        for seg in range(12):
            ...
            # Calculate and show min/max voltage
        all_voltages = [v for row in self.VoltageList for v in row if v >= 0]
        if all_voltages:
            min_v = min(all_voltages)
            max_v = max(all_voltages)
            avg_v = sum(all_voltages) / len(all_voltages)

            self.min_voltage_label.config(text=f"Min Voltage: {min_v:.3f} V")
            self.max_voltage_label.config(text=f"Max Voltage: {max_v:.3f} V")
            self.avg_voltage_label.config(text=f"Average Voltage: {avg_v:.3f} V")
        else:
            self.min_voltage_label.config(text="Min Voltage: -- V")
            self.max_voltage_label.config(text="Max Voltage: -- V")
            self.avg_voltage_label.config(text="Average Voltage: -- V")
